...
namespace tiledreader {
  ...
  TiledObjectShape TiledReaderLayerReader::MakePolygon(pugi::xml_node& polyroot, float origin_x, float origin_y, std::string area_name, std::string area_type) {
    ...
    std::string token;
    std::vector<std::pair<float,float>> verts;     //vector of 2d float points
    ...
    while (std::getline(iss, token, ' ')) {
      ...
      printf("  { %f, %f }, \n", ptx, pty);      //debug
      verts.push_back(std::pair<float,float>(ptx,pty));
    }
    printf("};\n");
    ...
    // a couple of cases:
    // if there are fewer than three vertices, error out.
    // if there's only one triangle, construct it and leave.
    if(verts.size() < 3) {
      printf("*** ERROR: too few vertices to make a triangle\n");
      poly = TiledObjectShape();    //inits to unknown and empty, that's our error
      return poly;
    } else if(verts.size() == 3) {
      //exactly one triangle! just add it to poly and be done
      printf("Single triangle case!\n");
      poly.add_triangle(verts[0].first,verts[0].second,verts[1].first,verts[1].second,verts[2].first,verts[2].second);
    } else {
      // used to be libtess2 stuff, removing - made a release in case I want to go back. Instead let's do a zigzag tesselation,
      // assuming the polygon is a perimeter type contour. If it isn't, well, map editor did it rong and they should feel bad
      // may mess up on especially crinkly polygons but if it does it's not such a burden to have the editor redo them
      // right now assuming they go around the perimeter of the polygon,
      ...
      printf("Making zigzag tesselation for triangles\n");
      std::vector<std::pair<float,float>> newverts(verts.size());
      for(auto j = 0, offset = 0; j < verts.size(); j++) {
        //even j, use offset; odd j, use end-offset and advance offset
        int newind = ((j%2) == 0) ? offset : (verts.size()-1) - (offset++);
        printf("%d ", newind);    //debug
        newverts[j] = verts[newind];
      }
      printf("\n"); //debug

      // FEH! This breaks on the third river set.
      // IS it so bad to find out if a point is inside a simple polygon?
      // http://geomalgorithms.com/a03-_inclusion.html - check the Winding Number part
      // missing a crucial image!
      // https://web.archive.org/web/20150301001957/http://geomalgorithms.com/a03-_inclusion.html
      // will print to pdf
      // Dropbox/Private/programming/PointInPolygonWinding.pdf
      // I will grab his "C++" code and look at it
      
      // turn all that into triangles... or should we just send a vertex list? Hm, I'm inclined to do triangles bc
      // then the collision can go like "is it in the bounding box? If so, check the triangles"
      // which is another motive for making sure the shape has a nice close bounding box
      // assuming the triangles are generated by doing indices 0 1 2, 1 2 3, 2 3 4, ...
      // DOES THE CL/CCL SENSE MATTER? if so, we'll figure it out
      // see https://stackoverflow.com/questions/1165647/how-to-determine-if-a-list-of-polygon-points-are-in-clockwise-order, 
      // lhf answer that says "Find the vertex with smallest y (and largest x if there are ties). Let the vertex be A and the 
      // previous vertex in the list be B and the next vertex in the list be C. Now compute the sign of the cross product of AB and AC."
      // - point must be on the convex hull (wiki page mentions lhf's method as one way to find such a point)
      // For a triangle, all of them are, so can use any point
      // see also https://en.wikipedia.org/wiki/Curve_orientation#Orientation_of_a_simple_polygon
      // DO SOME TESTS WITH CCL ONCE WE HAVE COLLISIONS GOING
      for(auto j = 0; j < newverts.size() - 2; j++) {
        poly.add_triangle(newverts[j].first, newverts[j].second,
                          newverts[j+1].first, newverts[j+1].second,
                          newverts[j+2].first, newverts[j+2].second);
      }
    }

    //debug: write triangles out as something we can initialize vectors with and see what it looks like
    ...
    //can use verts for this? Or triangles
